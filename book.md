...
- [第2章 TTY・PTY・ターミナルエミュレータを整理する](#第2章-ttyptYターミナルエミュレータを整理する)
- [第3章 キーボードからモニターまでの13層](#第3章-キーボードからモニターまでの13層)
- [第4章 使用する技術とツール](#第4章-使用する技術とツール)
- [第5章 Step 1 — 黒いウィンドウを出す](#第5章-step-1--黒いウィンドウを出す)
- [第6章 Step 2 — フォントを読み込んで文字を表示する](#第6章-step-2--フォントを読み込んで文字を表示する)
- [第7章 Step 3 — キーボード入力を受け取る](#第7章-step-3--キーボード入力を受け取る)
- [第8章 Step 4 — PTYを開いてシェルを起動する](#第8章-step-4--ptyを開いてシェルを起動する)
- [第9章 Step 5 — 入出力を接続する](#第9章-step-5--入出力を接続する)
- [第10章 Step 6 — カーソルと仕上げ](#第10章-step-6--カーソルと仕上げ)
...
# 第2章 TTY・PTY・ターミナルエミュレータを整理する

この章では、混同しやすい TTY・PTY・ターミナルエミュレータの関係を徹底的に整理する。

## 2.1 三者の関係

```
TTY (物理端末)     →  歴史の中で消えた
PTY (擬似端末)     →  TTY をソフトウェアで再現したカーネルの機能
ターミナルエミュレータ →  PTY を使って画面表示+キー入力を担当するGUIアプリ
```

一言で言えば:
- **ターミナルエミュレータ**は画面を描く**アプリケーション** (ユーザー空間)
- **PTY**はシェルとの通信を仲介する**カーネルの機能** (カーネル空間)

ターミナルエミュレータは PTY を**使う**。PTY はターミナルエミュレータの**道具**だ。

## 2.2 TTY とは何か — 物理端末の歴史

TTY は **teletypewriter（テレタイプライター）** の略。もともとは物理的な機械のことだ。

```
1960年代: テレタイプライター（紙に印字）
    ↓
1970年代: ビデオ端末 VT100（CRTディスプレイ）
    ↓
1980年代〜: 物理端末は消えた。でも名前だけ残った。
```

現代で「TTY」と言ったら、実質的には PTY のスレーブ側のデバイスを指していることが多い:

```bash
$ tty
/dev/ttys003    ← これはPTYのスレーブ側。物理端末ではない
```

### コラム: テレタイプの通信速度

テレタイプライターの通信速度は 110 baud（約10文字/秒）だった。現代のターミナルで `stty` コマンドを打つと `speed 38400 baud` のような表示が出るが、これはPTYが物理端末を模擬しているための名残で、実際のデータ転送速度とは無関係。

## 2.3 PTY とは何か — カーネル内の通信パイプ

PTY (Pseudo TTY = 偽のTTY) は、カーネル内に存在する通信の仕組みだ。**マスター側**と**スレーブ側**の2つの接続口を持つ。

```
ターミナルエミュレータ                カーネル内                     シェル
┌─────────┐     ┌──────────────────────────────────────┐     ┌──────────┐
│         │     │  マスター     line        スレーブ      │     │          │
│ myterm  │ ←→ │  (fd=3)  ← discipline →  ttys000    │ ←→ │ /bin/sh  │
│         │     │                                      │     │          │
└─────────┘     └──────────────────────────────────────┘     └──────────┘
ユーザー空間                   カーネル空間                     ユーザー空間
```

マスターもスレーブも**カーネルの中**にある。ターミナルエミュレータとシェルはその**両端にぶら下がっている**だけだ。
...
```
あなた ←→ [あなたの受話器 | 電話回線 | 友達の受話器] ←→ 友達
             マスター      line disc    スレーブ
```

「あなたの受話器」と「友達の受話器」は電話回線の両端についている部品であって、あなたや友達そのものではない。

| | マスター | スレーブ |
|---|---|---|
| **誰が使う** | ターミナルエミュレータ | シェル |
| **立場** | 「端末の外側」（人間に近い側） | 「端末の内側」（プログラムに近い側） |
| **操作** | ユーザーの入力を送る / 出力を受け取る | シェルが読む / 書く |
| **デバイス名** | 名前なし（ファイルディスクリプタの整数のみ） | `/dev/ttys000` (macOS), `/dev/pts/0` (Linux) |

マスターに書いたものがスレーブから読める。スレーブに書いたものがマスターから読める。間に line discipline が挟まってエコーや行編集を自動処理する。

### PTY はソケットに似ている

PTY の仕組みはネットワークのソケット通信と本質的に同じだ:

```
ソケット通信:
プログラムA ←→ [ソケットfd | カーネル | ソケットfd] ←→ プログラムB

PTY通信:
ターミナル  ←→ [マスターfd | カーネル | スレーブfd] ←→ シェル
```

どちらも:
- カーネルが仲介する**通信の仕組み**
- 両端に**ファイルディスクリプタ (fd)** がある
- `read()` / `write()` で通信する

違いは**間に line discipline が挟まるかどうか**:

| | ソケット | PTY |
|---|---|---|
| 間の処理 | 素通し（データをそのまま届ける） | line discipline（エコー、行編集、シグナル） |
| 用途 | 汎用のプロセス間/ネットワーク通信 | 端末エミュレーション専用 |
| API | `socket()`, `connect()` | `open("/dev/ptmx")`, `forkpty()` |

つまり PTY は「端末エミュレーション用の特殊なソケット」と考えてよい。

## 2.4 /dev 下のデバイスファイル — 実機で確認する

UNIX の思想「すべてはファイル」に従い、端末デバイスも `/dev/` 以下のファイルとして見える。

### macOS の /dev

```
/dev/
├── tty                  ← 特殊: 「自分が今繋がっている端末」を指す汎用デバイス
│
├── tty.Bluetooth-...    ← 物理シリアルポート (Bluetooth等)
├── tty.JBLTUNE125BT     ← Bluetoothデバイスもシリアルポート(tty)として見える
│
│  【旧式 BSD PTY — 互換性のため残存】
├── ptyp0 〜 ptypf       ← 旧式PTY のマスター側 (16個固定)
├── ttyp0 〜 ttypf       ← 旧式PTY のスレーブ側 (16個固定)
│
│  【現代 UNIX 98 PTY — 今はこちらを使う】
├── ptmx                 ← PTY マスター側の入口 (multiplexer)
├── ttys000              ← PTY スレーブ (使用中のものだけ動的に存在)
├── ttys001
├── ttys002
└── ...
```

旧式の `ttyp0`/`ptyp0` は1980年代の BSD に由来する。最大256個までしか作れない制約があった。
現代の `ptmx`/`ttys*` は1998年の UNIX 98 規格で導入され、動的に何個でも作れる。

`ptmx` の "mx" は **multiplexer（多重化装置）** の略。`open("/dev/ptmx")` するたびに新しい PTY ペアが生成される。
...
/dev/
├── tty                ← macOS と同じ
├── tty0 〜 tty63      ← 仮想コンソール (macOSには無い！)
├── ttyS0, ttyS1       ← 物理シリアルポート
│
├── ptmx               ← 現代PTY マスター入口
└── pts/               ← 現代PTY スレーブ (ディレクトリ内に動的生成)
    ├── 0
    ├── 1
    └── 2
```
...
| | macOS | Linux |
|---|---|---|
| 仮想コンソール (`tty0`〜`tty63`) | ない | ある (Ctrl+Alt+F1〜F6 で切替) |
| PTYスレーブの場所 | `/dev/ttys000` | `/dev/pts/0` |
| 旧式PTY (`ttyp`/`ptyp`) | 互換用に残存 | 最近のディストロでは削除済みが多い |

## 2.5 who コマンドで PTY を観察する

`who` コマンドは「誰がどの端末にログインしているか」を表示する。ここに PTY のスレーブ側が見える。

### macOS の例

```
$ who
mizuno   console    4月 7 19:16          ← GUIログイン（物理画面）
mizuno   ttys000    4月 8 15:06          ← ターミナルのタブ 1
mizuno   ttys001    4月 7 20:20          ← ターミナルのタブ 2
mizuno   ttys002    4月 7 22:47          ← ターミナルのタブ 3
mizuno   ttys003    4月 8 16:54          ← ターミナルのタブ 4
mizuno   ttys004    4月 8 10:37          ← ターミナルのタブ 5
mizuno   ttys005    4月 8 14:36          ← ターミナルのタブ 6
```

ターミナルのタブ/ウィンドウを6個開いている = PTY ペアが6組ある:

```
iTerm2/Terminal.app              カーネル                   シェル
┌─────────┐     ┌──────────────────────────────┐     ┌──────┐
│ タブ1    │ ←→ │ master ← line disc → ttys000 │ ←→ │ sh   │
│ タブ2    │ ←→ │ master ← line disc → ttys001 │ ←→ │ sh   │
│ タブ3    │ ←→ │ master ← line disc → ttys002 │ ←→ │ sh   │
│ タブ4    │ ←→ │ master ← line disc → ttys003 │ ←→ │ sh   │
│ タブ5    │ ←→ │ master ← line disc → ttys004 │ ←→ │ sh   │
│ タブ6    │ ←→ │ master ← line disc → ttys005 │ ←→ │ sh   │
└─────────┘     └──────────────────────────────┘     └──────┘
```

`who` にはスレーブ側（`ttys000` 等）しか出ない。マスター側はファイルディスクリプタ（ただの整数）としてのみ存在し、名前がないから `who` には表示されない。

`console` は PTY ではなく、Mac に直接ログインした（GUI ログイン画面でパスワードを入れた）記録。

### Linux (GCP VM) の例

```
$ who
yuki0322  pts/0   2026-04-08 07:56  (59.87.171.146)
yuki0322  pts/1   2026-04-08 07:56  (59.87.171.146)
yuki0322  pts/2   2026-04-08 07:56  (59.87.171.146)
```

同じIPから SSH 接続が3本ある。SSH 接続1本ごとに、サーバー側に PTY ペアが1組作られる:

```
あなたのMac                    GCP VM
┌──────────┐    SSH接続1     ┌────────────────────────────┐    ┌──────┐
│ タブA     │ ────────────→ │ master ← line disc → pts/0 │ ←→│ bash │
│ タブB     │ ────────────→ │ master ← line disc → pts/1 │ ←→│ bash │
│ タブC     │ ────────────→ │ master ← line disc → pts/2 │ ←→│ bash │
└──────────┘                └────────────────────────────┘    └──────┘
```

末尾の `(59.87.171.146)` は接続元のIPアドレス。不審なIPが表示されたら不正ログインの可能性がある。

### コラム: master/slave という用語について

近年、「master/slave」という用語は差別的だとして言い換えが進んでいる。Git は `master` → `main`、Python は `master/slave` → `parent/worker` に変更した。ただし PTY の `master/slave` は POSIX 規格に組み込まれているため、今のところ変わっていない。

## 2.6 すべての通信はカーネルを経由する

ターミナルエミュレータからコマンドを打つと、必ずカーネルを経由する:

```
「ls」と打って Enter を押した場合:

あなた → iTerm2 → カーネル(PTY) → /bin/sh → カーネル(fork/exec) → /bin/ls
                     ↑
                   ここを必ず通る
```

これは PTY に限った話ではない。**プログラム同士の通信はすべてカーネル経由**だ:
...
すべて `read()` / `write()` でカーネルに頼んでいる。プログラムは直接他のプログラムのメモリに触れない。カーネルが仲介役になって、安全にデータを受け渡す。プログラムがカーネルに仕事を頼む仕組みを**システムコール (syscall)** と呼ぶ。

### コラム: システムコールを覗く

`dtruss`（macOS）や `strace`（Linux）を使うと、プログラムが発行するシステムコールをリアルタイムで観察できる。myterm を完成させた後に試すと、PTY との `read`/`write` が実際に見える:

```bash
# macOS
sudo dtruss ./myterm 2>&1 | grep -E "read|write"

# Linux
strace -e read,write ./myterm
```

## 2.7 シェルの役割 — fork+exec する司令塔

PTY の先にいる `/bin/sh`（シェル）は、コマンドを**自分では実行しない**。シェルの仕事は**司令塔**として、別のプログラムを起動することだ。

### `cat file.txt` と打った場合の全体の流れ

```
あなた
  │ キーボードで "cat file.txt" + Enter
  ▼
myterm (ターミナルエミュレータ)
  │ write(master_fd, "cat file.txt\r")
  ▼
カーネル (PTY の line discipline)
  │ エコー + 行編集
  ▼
/bin/sh (シェル)                    ← ここが司令塔
  │ "cat file.txt" という文字列を受け取る
  │ スペースで分割: コマンド="cat", 引数="file.txt"
  │ fork() で子プロセスを作る
  │ execl("/bin/cat", "cat", "file.txt", NULL)
  ▼
/bin/cat (新しいプロセス)
  │ file.txt を開いて中身を stdout に write()
  ▼
カーネル (PTY)
  │ スレーブ → line discipline → マスター
  ▼
myterm
  │ read(master_fd) で結果を受け取る
  │ 画面に描画
  ▼
あなたの目に表示される
```
...
```
1. 文字列を受け取る     "cat file.txt"
2. 解析する            コマンド名 = "cat", 引数 = "file.txt"
3. fork() する         自分のコピー（子プロセス）を作る
4. exec() する         子プロセスを /bin/cat に置き換える
5. waitpid() する      cat の終了を待つ
6. プロンプトを出す     "$ " を表示して次の入力を待つ
```

sh は司令塔であって、実際のファイル操作やテキスト処理は `cat`, `ls`, `grep` などの個別プログラムがやる。

### 例外: シェル組み込みコマンド

一部のコマンドは sh 自身が直接実行する（fork+exec しない）:

| コマンド | なぜ組み込みか |
|---------|--------------|
| `cd` | カレントディレクトリは**プロセスごとの状態**。子プロセスで cd しても親(sh)に反映されない |
| `export` | 環境変数は sh 自身が管理している |
| `exit` | sh 自身が終了する必要がある |
| `echo` | 頻繁に使うので外部コマンドだと遅い（外部版 `/bin/echo` も存在する） |

```bash
# 組み込みか外部コマンドか確認する方法
$ type cd
cd is a shell builtin       ← sh が直接実行

$ type cat
cat is /bin/cat             ← 別プログラム。sh が fork+exec で起動
```

`ls | grep foo | sort` を実行すると、sh 含めて4プロセスが同時に動く。UNIX は小さなプログラムを組み合わせる思想なので、プロセス生成が非常に軽量に設計されている。

---

# 第3章 キーボードからモニターまでの13層
